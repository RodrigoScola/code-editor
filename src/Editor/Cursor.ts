...
export class Cursor {
	...
	paint(editor: DisplayComponent, canvas: Canvas) {
		...
		canvas.fillRect(relativePosition, this.style.setDisplay(content[this.column]));
	}
	...
	ensureVisible(viewPort: ViewPort) {
		...
		const lastVisibleColumn = viewPort.firstColumn + viewPort.visibleColumns - 1;
		if (this.column < viewPort.firstColumn) {
			viewPort.firstColumn = this.column;
		} else if (this.column > lastVisibleColumn) {
			viewPort.firstColumn = Math.max(0, this.column - viewPort.visibleColumns + 1);
		}
	}
